# Electric showcase — basic React + TypeScript template

Demo business: **Parked + Polished**. Starting ideas: detailing, fitness, product-led services.

Cobalt colour, large display typography, package cards and a featured-image carousel with thumbnails.

This is one of four independent client website templates for Created by Alvin. All four photo-showcase layouts are included; this demo defaults to `spotlight`. All business-specific content is configurable, so the sample industry is a starting point rather than a limitation.
